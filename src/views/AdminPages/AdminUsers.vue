...
const usersData = () => {
  axiosInstance.get(`http://13.125.169.9:80/user/all`)
  .then(response => {
...
const addUser = async () => {
  try {
    await axiosInstance.post('http://13.125.169.9:80/user', newUser.value);
    usersData();
...
const saveEditedUser = async (user) => {
  try {
    ...
    await axiosInstance.put(`http://13.125.169.9:80/user/${user.id}`,newUser);
...
const deleteUser = async (id) => {
  try {
    await axiosInstance.delete(`http://13.125.169.9:80/user/${id}`);
    usersData();
  } catch (error) {
